...
def trainWithRollout(sample, policy_net, target_net, optimizer, H, env_model, temp, decay, errors_per_step, overall_errors, weights_per_step, effective_planning_horizons, td_errors_diff, td_errors_direction_diff, full_target_errors, uncertainty_measures, dimension):
    ...
    if H == 1:
        ...
        for i in range(BATCH_SIZE):
            ...
            ## change this to fixed weights
            str_temp = "1" if temp == 1.0 else str(temp)
            file_name = f"weights/2023_07_05_acrobot_one_state_oracle_h{H}.dim.{dimension}.t.{str_temp}_weights"
            # Read the results file and extract the values
            with open(file_name, "r") as f:
                line = f.readline()
                weights = line.strip().split("\t")

            # Convert the values to float if needed
            weights = [float(value) for value in weights]

            # Print the list of weight values
            ...
            if t % TRAINING_FREQ == 0 and sample_policy is not None:
                if target_off:
                    errors_per_step, overall_errors, weights_per_step, effective_planning_horizons, td_errors_diff, td_errors_direction_diff, full_target_errors, uncertainty_measures = trainWithRollout(sample_policy, policy_net, policy_net, optimizer, rollout_constant, env_model, temp=temp, decay=decay, 
                                                                                                                                      ...
                                                                                                                                      td_errors_diff=td_errors_diff, td_errors_direction_diff=td_errors_direction_diff, full_target_errors=full_target_errors, uncertainty_measures=uncertainty_measures, dimension=env_hidden_size)
                else:
