...
class DoneModel(nn.Module):
    ...
    def step(self, action):
        one_hot_action = torch.eye(self.action_size)[action.squeeze().long()]
        state_action_pair = torch.cat((self.state, one_hot_action), dim=-1)
        predicted_done = self.single_forward(state_action_pair)
...
def trainWithRollout(sample, policy_net, target_net, optimizer, H):
    ...
    for i in range(BATCH_SIZE):
        ...
        done_model.load_state(initial_state)
        ...
        done_model.load_state(next_state)

        for h in range(1, H):
            if done < 1:
                action = choose_greedy_action(state, policy_net)
                next_state, reward = env_model.step(action)
                done = done_model.step(action)
                done = 1 if done >= 0.5 else 0
                env_model.load_state(next_state)
                done_model.load_state(next_state)
