...
class QNetwork(pl.LightningModule, nn.Module):
    def __init__(self,
        ...
        self.encoder_act = nn.Sequential(
        ...
        self.decoder_act = nn.Sequential(
            nn.Linear(int(hidden_dim), int(hidden_dim / 2)),
            nn.ReLU(),
            self.dropout,
    ...
    def get_action_value(self, input):
        emb_objs = self.encoder_act(input)
        emb_objs_vector, _ = torch.max(emb_objs, dim=1)

        reward = self.decoder_act(emb_objs_vector)
...
def world_dynamics(t, replay_start_size, num_actions, s_cont, env, policy_net):
    ...
    if t < replay_start_size:
        ...
        if np.random.binomial(1, epsilon) == 1:
            ...
            with torch.no_grad():
                action = policy_net.get_action_value(s_cont).max(1)[1].view(1, 1)

    # Act according to the action and observe the transition and reward
    reward, terminated = env.act(action)
    terminated = 0 if terminated == False else 1
...
def train(sample, policy_net, target_net, optimizer):
    ...
    # Use DQN to compute loss
    Q_s_a = policy_net.get_action_value(states).gather(1, actions)
    ...
    if len(none_terminal_next_states) != 0:
        Q_s_prime_a_prime[none_terminal_next_state_index] = target_net.get_action_value(none_terminal_next_states).detach().max(1)[0].unsqueeze(1)
