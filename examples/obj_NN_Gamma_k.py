...
class QNetwork(pl.LightningModule, nn.Module):
    def __init__(self,
        ...
        self.output = nn.Linear(int(hidden_dim / 2), num_actions)
    ...
    def forward(self, s, debug=False):
        ...
        dim = int(reward.size()[1])
        reward1 = reward[:, 0:int(3*dim/4)]
        reward2 = reward[:, int(3*dim/4):]
        reward2 = torch.pow(GAMMA, reward2)
        reward = torch.cat((reward1, reward2), 1)
